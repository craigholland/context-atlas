...
def _nearest_owner_file(repo_root: Path, path: Path, owner_filename: str) -> Path | None:
...
def _is_governed_file(path: Path, governed_roots: tuple[Path, ...], suffixes: tuple[str, ...]) -> bool:
...
def _has_meaningful_owner_change(
    ...
    *,
    ...
    return _normalize_last_verified_block(old_text) != _normalize_last_verified_block(new_text)
...
def main() -> int:
    ...
    missing_updates = sorted(
        owner_file for owner_file in required_owner_files if owner_file not in changed_rel
    )
    ...
    if metadata_only_updates:
        has_failures = True
        print("[check_ai_docs] FAIL: owner files were updated only in Last Verified (CI):")
