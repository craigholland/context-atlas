...
def _shell_command_prefix() -> list[str]:
    ...
    Most CI examples in Craig-style repos assume bash-compatible commands.
    On Windows, `pwsh` is a reasonable fallback if bash is unavailable.
    """
    ...
    raise RuntimeError("No supported shell executable found (expected bash or PowerShell).")
