...
def _resolve_import_from_base(source_root: Path, path: Path, node: ast.ImportFrom) -> str | None:
    if node.level == 0:
        return node.module
...
def _iter_import_from_targets(source_root: Path, path: Path, node: ast.ImportFrom) -> list[str]:
