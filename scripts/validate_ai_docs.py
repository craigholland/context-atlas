...
def _validate_headings(text: str, *, owner_rel: Path) -> list[str]:
    ...
    for heading in REQUIRED_HEADINGS:
        if heading not in text:
            errors.append(
                f"{owner_rel.as_posix()}: missing required heading '{heading}'"
            )
    return errors


def _validate_non_empty_section(
    text: str, heading: str, *, owner_rel: Path
) -> list[str]:
    pattern = re.compile(
...
def _validate_verification_contract(text: str, *, owner_rel: Path) -> list[str]:
    ...
    if not block_match:
        return [
            f"{owner_rel.as_posix()}: Verification Contract block missing or malformed"
        ]
...
def _validate_owner_file(repo_root: Path, owner_rel: Path) -> list[str]:
    ...
    errors.extend(
        _validate_non_empty_section(text, "## File Index", owner_rel=owner_rel)
    )
    errors.extend(
