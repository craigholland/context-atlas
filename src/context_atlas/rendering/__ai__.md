...
steps:
  - name: compile_rendering
    run: |
      python -m compileall src/context_atlas/rendering

  - name: unit_tests
    run: |
      python -m pytest tests/test_budget_and_compression.py tests/test_context_assembly_service.py tests/test_packet_rendering.py tests/test_trace_rendering.py

  - name: import_sanity
    run: |
      python -c "import sys; sys.path.insert(0, 'src'); from context_atlas.rendering import render_packet_context, render_packet_inspection, render_trace_inspection"
```
