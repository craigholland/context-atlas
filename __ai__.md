...
- Top-level rough-draft Epic product definitions may also live directly under `docs/Planning/` while their scope is still being pressure-tested before Story/Task decomposition begins, but once a horizon graduates into a numbered Epic directory the folder-local product definition, Story docs, and later `Stories/Tasks/` stack become the live planning surface.
- When those rough-draft Epics are review-response planning surfaces, they should stay explicit about whether they are targeting quick wins, runtime-model strengthening, deeper architectural tensions, AI governance-system evolution, or consumer-packaging and distribution so contributors do not collapse distinct planning horizons back into one omnibus follow-up.
- Review-response rough-draft Epics may also carry a distinct consumer-packaging and distribution horizon when the next major question is what should ship for product users versus what should remain contributor- or governance-facing repo infrastructure.
- Low-hanging review-response planning may also carry bounded contributor-trust fixes such as Linux-first CI and executable contract-command alignment when the work is surface cleanup rather than deeper governance redesign.
- Deeper-architecture review-response planning should stay limited to code-level `src/context_atlas/` tensions, while generated-runtime namespacing and platform policy belong to the agentic-runtime horizon and repo-local authority precedence belongs to the AI-governance horizon.
- The current review-response Epic sequence should default to quick wins first, then agentic runtime strengthening, then deeper code architecture, then AI governance evolution, and finally consumer packaging unless release pressure or a bounded parallelization decision justifies a different order.
...
- `docs/Planning/`: top-level rough-draft Epic product definitions are acceptable here when they are intentionally capturing candidate next major surfaces before a full Story/Task stack exists, but once an Epic is actively decomposed it should move into a numbered folder such as `docs/Planning/013_Cleanup/`.
- `docs/Planning/013_Cleanup/`: is now the live Cleanup Epic stack, so future Cleanup decomposition should extend its `Stories/` and `Stories/Tasks/` surfaces rather than reintroducing new top-level cleanup planning docs.
- `docs/Planning/`: the current review-response planning set is intentionally split into cleanup, agentic runtime, deeper code architecture, AI governance, and consumer packaging so contributors should update the right horizon instead of smearing one decision across all five.
- `docs/Planning/README.md`: should keep the current review-response Epic set visible in one place, should make it obvious when one horizon has graduated into a numbered Epic/Story stack, and should recommend a working sequence so contributors know which horizon is expected to move first when all five remain active together.
