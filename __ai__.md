...
## Last Verified (CI)
- commit: 95dfedebe23408923589a0215189a94cd63308ef
- timestamp_utc: 2026-04-21T01:33:32Z
- verified_by: ci
...
- Hardening Story 1 now treats canonical source ownership and lexical index-shape construction as separate outward adapter concerns; later retrieval reuse should extend the `retrieval/registry.py` plus `retrieval/indexing.py` baseline rather than collapsing those concerns back into one lexical module.
- Hardening Task 1.1 now also establishes registry-revision-aligned baseline snapshot reuse inside `retrieval/lexical.py`; later retrieval hardening should extend that one cache boundary rather than inventing parallel source-tracking or invalidation mechanisms elsewhere.
- Hardening Task 1.2 now also treats the supported steady-state retrieval reuse shape as one registry-revision-aligned snapshot carrying both corpus-wide IDF state and source-side TF-IDF vectors, and later work should extend that unified snapshot rather than splitting reuse across parallel caches.
- Hardening Task 1.2 is now the point where the "cached IDF, rebuilt documents" gap is considered closed; later Story 1 work should focus on proof and reviewability of that unified snapshot path rather than re-opening source-side vector ownership.
- Hardening Task 1.3 now also establishes that warm-cache TF-IDF retrieval still crosses one fresh registry source listing per call and preserves deterministic repeated-query semantics; later Story 1 work should add proof and observability on top of that shared path rather than inventing a second warm-cache engine mode.
- Hardening Task 1.3 is now also the point where repeated-query boundary and determinism concerns are considered settled for Story 1; later Task 1.4 work should focus on making that reuse path reviewable rather than re-litigating registry ownership or semantic stability.
- Hardening Task 1.4 now treats the shared retrieval-completed event plus regression coverage as the preferred bounded proof surface for Story 1; later retrieval reviewability work should extend that `index_snapshot_state`-style signal rather than introducing benchmark theater or a proof-only retrieval path.
- Hardening Task 1.4 is now also the point where Story 1's proof baseline is considered settled: one repeated-query regression bundle plus the shared retrieval-completed `rebuilt`/`warm` signal should be treated as sufficient unless a later Story explicitly proves a new need.
