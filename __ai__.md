...
- `docs/Planning/013_Cleanup/Stories/`: Story 3 currently carries a bounded Canon-leak inventory contract: cleanup should stay tied to the explicitly named Context Atlas/Codex leak set in AgenticDevelopment, Architecture, and Ontology canon surfaces instead of broad-brush abstraction work.
- `docs/Planning/013_Cleanup/Stories/`: Story 3 also carries a classification contract: each named Canon leak should be treated as either `generalize in Canon`, `reroute downstream`, or `keep intentionally` rather than being resolved ad hoc in later slices.
- Story 3 cleanup slices that touch portable Canon should prefer generic runtime/governance wording over Context Atlas or Codex-specific examples unless a remaining local example is explicitly justified as intentional case-study context.
- Story 3 Task 3.2 now also carries a replacement boundary: AgenticDevelopment and Architecture canon examples should be generalized first, while Ontology-branded examples remain explicitly classified work rather than automatic spillover.
- Story 3 Task 3.3 now also carries a boundary-reinforcement rule: Canon entry surfaces may point readers to the current repository's Identity landing docs, but the expected descent path is profile first, then binding/manifest detail, then the current runtime-binding README rather than a direct jump from portable canon to runtime surfaces.
- Story 3 Task 3.4 now also carries a readability-closeout rule: after leak cleanup, the portable entry surfaces should still teach the model cleanly, and any remaining local examples must read as explicitly classified exceptions rather than accidental default assumptions.
