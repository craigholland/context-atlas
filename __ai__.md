...
- `.codex/modes/`: generated mode surfaces must not absorb `Constraints`, `Non-Goals`, or `Related Artifacts` bullets from upstream transition docs into entry/exit sections; if a mode surface shows that bleed, the fix belongs in the upstream extractor/generator path rather than a hand edit to the generated file.
- `docs/Authoritative/Canon/AgenticDevelopment/` and `docs/Authoritative/Identity/AgenticDevelopment/Bindings/Modes/`: when those docs are projected into runtime-facing mode files, heading hierarchy must remain authoritative so portable or project-level constraints do not leak into generated operational sections like entry and exit conditions.
- `docs/Planning/013_Cleanup/Stories/story_4_immediate_generated_surface_defects.md`: Story 4 should stay bounded to the confirmed section-boundary bleed defect class and may refresh the one affected generated file alongside the upstream fix when that is the smallest way to keep the drift checker truthful.
- `tests/test_codex_materialization.py`: the generated-runtime regression surface should keep at least one direct proof that mode-section extraction stops at same-or-higher heading boundaries, so the Story 4 defect class does not have to be rediscovered through a visibly broken `.codex/modes/` file.
