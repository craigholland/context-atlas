...
- Cross-links between routing surfaces should be labeled by audience and purpose, so deeper canon/runtime/governance routes stay reachable without re-claiming the product path as a hidden prerequisite.
...
- Contributor/governance operational steps should live in the surfaces that own them; repo-edge product-facing docs should route readers there rather than carrying detailed refresh or verification commands inline.
- For the current Codex materialization stack, contributor refresh commands should live under `docs/Authoritative/Identity/AgenticDevelopment/Materializations/Codex/` rather than in product-facing README or guide surfaces.
...
- `docs/Planning/013_Cleanup/Stories/`: Story 2 also carries a settled index-hierarchy contract: `README.md` is the repo map, `docs/README.md` is the documentation route splitter, and `docs/Guides/README.md` is the product guide hub; cross-links between those surfaces should be audience-labeled instead of reopening one blended entry path.
- `docs/Planning/013_Cleanup/Stories/`: Story 2 now also carries a settled contributor-containment contract: repo-edge product-facing docs should route contributors to owning Codex materialization/governance surfaces rather than carrying detailed refresh or drift-check commands inline.
