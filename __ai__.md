...
## Last Verified (CI)
- commit: cd14f5471bb397c830d9e9611771f3fe647b9f0d
- timestamp_utc: 2026-04-19T22:11:04Z
- verified_by: ci
...
- The current active MVP task is Story 7 Task 7.3, which is clarifying the supported configuration surface by deciding which starter defaults remain internal and which become env-backed runtime knobs.
- The current Task 7.3 configuration-surface decision keeps ranking authority tables, ranking signal names, memory-scoring semantics, and canonical slot identifiers internal by design; only the starter memory-budget split is currently approved for promotion into the supported env-backed surface.
- The supported env-backed surface now includes the starter memory-budget split as `CONTEXT_ATLAS_DEFAULT_MEMORY_BUDGET_FRACTION`; repo-level docs should keep that visible while continuing to treat ranking authority tables, memory-scoring semantics, and canonical slot identifiers as internal.
- Product-facing docs should now name `CONTEXT_ATLAS_DEFAULT_MEMORY_BUDGET_FRACTION` explicitly when describing the starter documents-vs-memory split instead of describing that knob only generically.
