...
## Last Verified (CI)
- commit: b093056c82bade75131dbd8d4d0988f1219f81b7
- timestamp_utc: 2026-04-22T14:24:19Z
- verified_by: ci
...
- Active planning work should now live directly under `docs/Planning/`, while completed Epics and their downstream Story/Task decomposition should move under `docs/Planning/completed/` so future intent and historical execution do not blur together.
- The completed agentic-development planning stack now lives under `docs/Planning/completed/Agentic/`; contributors should keep those archived Epic, Story, and Task layers aligned rather than treating the Task docs as a separate planning system.
- The completed post-MVP hardening planning stack now lives under `docs/Planning/completed/Hardening/`; contributors referencing that archive should keep retrieval-index reuse, duplicate-detection quality, token-estimation decisions, and budget/compression truthfulness grouped as one shared engine-hardening program rather than scattering them across unrelated docs.
- The completed hardening Epic is specifically the context-assembly hardening surface under `docs/Planning/completed/Hardening/context_assembly_hardening_product_definition.md`.
- The completed hardening archive also includes Story docs under `docs/Planning/completed/Hardening/Stories/` and Task PR plans under `docs/Planning/completed/Hardening/Stories/Tasks/`; later follow-on work should inherit their architectural boundaries and execution evidence rather than reopening them as if they were still active planning.
...
- The project-specific runtime-capacity planning input should now live at `docs/Authoritative/Identity/AgenticDevelopment/runtime_capacity.yaml`, with the human update and trust model documented in `docs/Authoritative/Identity/AgenticDevelopment/Bindings/runtime_capacity.md`; contributors should not create alternate planning-capacity sources in Story docs, prompts, or runtime-specific folders.
- The project-specific runtime materialization roster should now bind through `docs/Authoritative/Identity/AgenticDevelopment/materialization_manifest.yaml`; contributors should not let `.codex/`, `.agents/`, or platform-specific templates become the hidden source of truth for which roles, parent agents, modes, protocols, specialists, or skills are currently in scope.
- The materialization manifest should keep parent direct skills, parent/specialist mode participation, parent protocol ownership or participation, delegated specialists, and specialist-attached skills together in one machine-readable attachment map while preserving those semantics as distinct concepts rather than flattening them into one generic list.
- The same manifest may give an implementation parent a bounded planning skill when that skill is only for decomposing an already-assigned Task into PR-sized execution slices; broader Epic, Story, and Task planning ownership should remain with the Planner/Decomp role unless the Identity bindings change explicitly.
- The project-specific role roster for agentic development should live under `docs/Authoritative/Identity/AgenticDevelopment/Bindings/Roles/Role-Model.md`; contributors should not let specialist labels, runtime worker names, or tool-specific naming conventions become a shadow role model, and they should not imply a dedicated frontend/UI role until the repository actually owns that kind of product surface.
...
- The project-specific accountability and ownership surface for agentic-development roles should live under `docs/Authoritative/Identity/AgenticDevelopment/Bindings/Roles/Role-Accountability-Matrix.md`; contributors should keep specialist participation subordinate to parent-owned role accountability instead of creating a second ownership layer.
- The project-specific authority surface for agentic-development roles should live under `docs/Authoritative/Identity/AgenticDevelopment/Bindings/Roles/Role-Authority-Matrix.md`; contributors should keep approval, merge, release, and workflow authority explicit instead of letting it drift out of ownership or implementation participation.
- The project-specific role-to-parent binding for agentic development should live under `docs/Authoritative/Identity/AgenticDevelopment/Bindings/Roles/Role-Agent-Binding-Model.md`; contributors should not let specialists, runtime files, or worker labels become substitute expressions of top-level project accountability.
- The project-specific mode model for agentic development should live under `docs/Authoritative/Identity/AgenticDevelopment/Bindings/Modes/Mode-Model.md`; contributors should keep modes as execution states rather than letting them collapse into role aliases, protocol names, or runtime prompt habits.
...
- The Context Atlas Codex binding should express its concrete runtime layout through the Identity-layer docs under `docs/Authoritative/Identity/AgenticDevelopment/Materializations/Codex/`; later `.codex/` and `.agents/skills/` assets should inherit that layout instead of inventing ad hoc folder conventions from memory.
...
- The same Codex binding should keep it explicit that `.codex/` and `.agents/skills/` assets are generated or regenerated derived surfaces; local edits there may be overwritten, and durable semantic changes should be made upstream in `docs/Authoritative/Canon/` or `docs/Authoritative/Identity/` first.
<<<<<<< codex/materialization-enforcement-planning
