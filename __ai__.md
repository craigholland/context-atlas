...
- `docs/Planning/013_Cleanup/Stories/`: Story 1 currently carries two settled starter-path contracts that downstream cleanup work should preserve: the Python `3.12+` runtime floor is surfaced early, and the first-run starter sample now points at the checked-in sample repo docs plus one bounded starter output artifact rather than the repo root `docs/` tree.
- `docs/Planning/013_Cleanup/Stories/`: Story 1 now also carries a settled guide-order contract: `Getting Started` owns the first-run baseline, `Context Atlas Tour` is the next system-layer step, and workflow-specific guides are downstream variations rather than alternate first-run entrypoints.
- `docs/Planning/013_Cleanup/Stories/`: Story 1 now also carries a settled evaluator-surface alignment contract: `README.md` remains the repo map, `docs/Guides/` remains the onboarding/tour layer, `examples/` remains the runnable companion layer, and `docs/Release/` remains shipped-history context rather than the primary first-run help path.
...
- Product-facing guides and example companions should stay aligned around one runnable starter corpus; they should not imply that the repo root `docs/` tree is the default first-run sample unless that path is intentionally supported and validated.
- The product-facing starter path should expose at least one bounded packet/trace-shaped output sample derived from a real starter run; when stable sample artifacts normalize volatile IDs, the surrounding copy should say so explicitly instead of presenting them as literal fixed values.
- Product-facing guides should describe current surfaced behavior in present-tense product language rather than Story/Task labels or implementation-history shorthand, unless a planning/history reference is itself the subject of the guide.
- The guide layer should preserve one explicit reading order: `Getting Started` for first-run setup, `Context Atlas Tour` for the next system layer, and workflow-specific guides only after that shared starter baseline is clear.
- Product-facing example READMEs should describe current runnable workflow behavior in present-tense product language too; they should not rely on Story-number or hardening-era shorthand to explain what the current evaluator path is.
- Release-history surfaces should stay clearly secondary to the current guide and example path; shipped release notes may summarize what was published, but they should not become the primary first-run help surface for the current repository state.
