...
- Top-level rough-draft Epic product definitions may also live directly under `docs/Planning/` while their scope is still being pressure-tested before Story/Task decomposition begins.
...
- `docs/Planning/`: top-level rough-draft Epic product definitions are acceptable here when they are intentionally capturing candidate next major surfaces before a full Story/Task stack exists.
- `docs/Planning/`: the current review-response planning set is intentionally split into quick wins, agentic runtime, deeper code architecture, AI governance, and consumer packaging so contributors should update the right horizon instead of smearing one decision across all five.
- `docs/Planning/README.md`: should keep the current rough-draft Epic set visible in one place and should recommend a working sequence so contributors know which horizon is expected to move first when all five remain active together.
