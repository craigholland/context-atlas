...
This is the runtime-facing orientation surface for the Context Atlas Codex
materialization. It is downstream of the portable AgenticDevelopment canon,
the Context Atlas Identity bindings, and the Codex materialization rules.

Use this file to orient to the local runtime surface, then move quickly into
the more specific role, agent, mode, protocol, and skill files instead of
treating this file as the only definition source.
...
3. Read the parent-agent descriptor in `.codex/agents/` for the accountable
   actor bound to that role.
4. If work is delegated, read the relevant specialist descriptor in
   `.codex/agents/`.
5. Read the active mode under `.codex/modes/` and the governing workflow under
   `.codex/protocols/`.
6. Read only the attached skills needed for the current bounded work under
   `.agents/skills/`.
7. Use `.codex/config.toml` for runtime roots, manifest traceability, and
   planning-capacity defaults.
